...
- 若设置了 `OUTPUT_FILE`，使用 `OUTPUT_FILE` 指定路径；
- 若未设置 `OUTPUT_FILE`，默认使用 `$TMPDIR/adguard_home_rules.txt`；
- 当 `TMPDIR` 未设置时，回退到 `/tmp/adguard_home_rules.txt`。
...
- 本仓库中的脚本与工作流文件采用 [GPLv3 许可证](LICENSE)。
- 生成规则依赖第三方数据源 [Loyalsoldier/surge-rules](https://github.com/Loyalsoldier/surge-rules)，使用或再分发时请同时遵守其仓库声明的许可与使用条款。
- 发布到 Releases 的 `adguard_home_rules.txt` 为基于第三方数据源生成的规则产物，其分发与使用以第三方数据源许可条款为准。
